...
import { X, Save, Edit3, Calculator, Wallet, DollarSign, HeartHandshake, Briefcase } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { STAFF_FIELDS } from '../../../utils/payrollConfig'; // Importamos la configuración

// Componente Input Externo (Para no perder foco)
const MoneyInput = ({ label, value, onChange, readOnlyLabel, isHighlight, isDeduction }) => (
  <div>
    <label className={`block text-xs font-bold mb-1 uppercase tracking-wide ${isDeduction ? 'text-red-600' : isHighlight ? 'text-blue-600' : 'text-slate-500'}`}>{label}</label>
    <div className="relative group">
      <span className={`absolute left-3 top-2.5 font-bold ${isDeduction ? 'text-red-400' : isHighlight ? 'text-blue-500' : 'text-slate-400 group-focus-within:text-blue-500'}`}>S/.</span>
      <input 
        type="number" step="0.01"
        value={value}
        onChange={onChange}
        onFocus={(e) => e.target.select()}
        className={`w-full pl-9 p-2.5 rounded-xl border focus:outline-none focus:ring-2 font-bold shadow-sm transition-all 
          ${isDeduction 
            ? 'border-red-200 bg-red-50 text-red-900 focus:ring-red-500' 
            : isHighlight 
...
const AdjustmentsModal = ({ isOpen, onClose, onSave, initialData, autoCalculatedData, workerName }) => {
  const isStaff = autoCalculatedData?.type === 'staff';
  const [formData, setFormData] = useState({});
  
  // Estados exclusivos de Obrero
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (isStaff) {
        // --- CARGA DINÁMICA PARA STAFF ---
        const newFormData = {};
        STAFF_FIELDS.forEach(section => {
          section.fields.forEach(field => {
            // Prioridad: 1. Guardado Manual, 2. Cálculo Automático, 3. Cero
            newFormData[field.key] = initialData?.[field.key] !== undefined 
        ...
        // --- CARGA MANUAL PARA OBRERO (Mantenemos tu lógica exacta) ---
        const f = d.factors || {};
        setFactors({
  ...
  }, [isOpen, initialData, autoCalculatedData, isStaff]);
  ...
  const handleSubmit = (e) => {
    ...
    Object.keys(formData).forEach(key => {
        finalData[key] = Number(formData[key]);
    });
      ...
      <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-md overflow-y-auto">
        ...
        >
          ...
          <div className="bg-[#003366] px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-md">
            <div>
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {isStaff ? 'Edición Planilla Staff' : 'Edición Manual Obrero'}
              </h3>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
            ...
            {/* ---------------- INTERFAZ STAFF (DINÁMICA Y ORDENADA) ---------------- */}
            {isStaff ? (
              <div className="space-y-6">
                 <div className="bg-purple-50 p-4 rounded-xl border border-purple-100 flex items-center gap-4">
                    <div className="bg-white p-2 rounded-lg text-purple-600 shadow-sm"><Briefcase size={24}/></div>
                    <div>
                        <h4 className="font-bold text-purple-900 text-sm">Modo Staff</h4>
                        <p className="text-xs text-purple-600">Edita los conceptos según la Planilla General.</p>
                    </div>
                </div>

                {/* Renderizamos las secciones definidas en payrollConfig.js */}
                {STAFF_FIELDS.map((section, idx) => (
                  <div key={idx} className="space-y-3">
                    <h4 className={`text-sm font-bold border-b pb-2 flex items-center gap-2 ${section.title.includes('Descuentos') ? 'text-red-700 border-red-100' : 'text-slate-700 border-slate-100'}`}>
                      {section.title}
                    </h4>
                ...
                /* ---------------- INTERFAZ OBRERO (TU VERSIÓN CLÁSICA) ---------------- */
                <>
                    {/* JORNAL DIARIO */}
                       ...
                       <div className="flex gap-3 items-start">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-blue-600 max-w-[300px] leading-tight mt-1">
                              Al editar esto, se recalculan automáticamente todos los ingresos dependientes.
                            </p>
                          </div>
                       </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <MoneyInput label="Jornal Básico" value={formData.basicSalary} onChange={(e) => setFormData({...formData, basicSalary: e.target.value})}/>
                        <MoneyInput label="Dominical" value={formData.dominical} onChange={(e) => setFormData({...formData, dominical: e.target.value})}/>
                        <MoneyInput label="B.U.C." value={formData.buc} onChange={(e) => setFormData({...formData, buc: e.target.value})}/>
                        <MoneyInput label="Movilidad" value={formData.mobility} onChange={(e) => setFormData({...formData, mobility: e.target.value})}/>
                        <MoneyInput label="Asig. Escolar" value={formData.schoolAssign} onChange={(e) => setFormData({...formData, schoolAssign: e.target.value})}/>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 bg-purple-50/50 p-4 rounded-xl border border-purple-100">
                        <MoneyInput label="Vacaciones" value={formData.vacation} onChange={(e) => setFormData({...formData, vacation: e.target.value})}/>
                        <MoneyInput label="Gratificación" value={formData.gratification} onChange={(e) => setFormData({...formData, gratification: e.target.value})}/>
                        <MoneyInput label="Indemnización (CTS)" value={formData.indemnity} onChange={(e) => setFormData({...formData, indemnity: e.target.value})}/>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 bg-orange-50/50 p-4 rounded-xl border border-orange-100">
                        <MoneyInput label="Monto H.E. 60%" value={formData.amountHe60} onChange={(e) => setFormData({...formData, amountHe60: e.target.value})}/>
                        <MoneyInput label="Monto H.E. 100%" value={formData.amountHe100} onChange={(e) => setFormData({...formData, amountHe100: e.target.value})}/>
                        <MoneyInput label="Indemnización H.E." value={formData.indemnizacionHe} onChange={(e) => setFormData({...formData, indemnizacionHe: e.target.value})}/>
                        <div className="col-span-1 sm:col-span-2 md:col-span-1 flex gap-2">
                            <div className="w-16">
                                ...
                                <input type="number" min="0" step="1" value={formData.workedHolidayDays} onChange={(e) => handleHolidayDaysChange(e.target.value)} onFocus={(e) => e.target.select()} className="w-full p-2.5 rounded-xl border border-orange-200 text-center font-bold text-orange-900 focus:outline-none focus:ring-2 focus:ring-orange-500"/>
                            </div>
                            <div className="flex-1">
                                <MoneyInput label="Remun. Feriado" value={formData.holidayPay} onChange={(e) => setFormData({...formData, holidayPay: e.target.value})}/>
                            </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                         <MoneyInput label="Bonif. Voluntaria" value={formData.bonus} onChange={(e) => setFormData({...formData, bonus: e.target.value})}/>
                         <MoneyInput label="Viáticos (No Afecto)" value={formData.viaticos} onChange={(e) => setFormData({...formData, viaticos: e.target.value})}/>
                         <MoneyInput label="Descuento Vario" value={formData.deduction} onChange={(e) => setFormData({...formData, deduction: e.target.value})} isDeduction={true}/>
                      </div>
                      <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between gap-4">
                          <div className="text-xs text-slate-500"><span className="font-bold text-slate-700">Feriados NO TRABAJADOS (Descanso):</span><br/>Días feriados pagados en el básico pero no se laboraron.</div>
                          <input type="number" min="0" max="7" value={formData.paidHolidays} onChange={(e) => setFormData({...formData, paidHolidays: e.target.value})} onFocus={(e) => e.target.select()} className="w-20 p-2 text-center font-bold border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"/>
                      </div>
                    </div>
                </>
            )}
            ...
            <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 mt-4 sticky bottom-0 bg-white py-4">
              <button type="button" onClick={onClose} className="px-5 py-2.5 text-slate-600 font-bold hover:bg-slate-50 rounded-lg text-sm transition-colors border border-slate-200">Cancelar</button>
              <button type="submit" className="bg-[#003366] text-white px-8 py-2.5 rounded-lg font-bold text-sm hover:bg-blue-900 transition-all shadow-lg flex items-center gap-2"><Save size={18} /> Guardar</button>
            </div>
          </form>
        </motion.div>
      </div>
