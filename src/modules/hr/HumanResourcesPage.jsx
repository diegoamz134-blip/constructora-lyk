...
import { 
  ...
  ShieldCheck, Banknote, HardHat, CalendarX, Pencil
} from 'lucide-react';
...
const HumanResourcesPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Cargar Adelantos (SOLO PARA STAFF)
      if (activeTab === 'staff') {
        let advancesQuery = supabase.from('advances').select('employee_id, amount').eq('status', 'Pendiente');
        const { data: advancesData } = await advancesQuery;
        
        const advancesMap = {};
        advancesData?.forEach(adv => {
          if (adv.employee_id) advancesMap[adv.employee_id] = (advancesMap[adv.employee_id] || 0) + adv.amount;
        });
        setAdvances(advancesMap);
      } else {
        setAdvances({}); // Limpiar adelantos si estamos en obreros
      }
  ...
  // Solo calcular alertas de contrato para Staff
  const contractsAlertCount = activeTab === 'staff' ? people.filter(p => {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Tarjeta Dinámica Central */}
        {activeTab === 'staff' && contractsAlertCount > 0 ? (
          <div className="bg-white p-6 rounded-2xl border border-red-100 shadow-sm flex items-center justify-between relative overflow-hidden">
              ...
              <div>
                  <p className="text-red-400 text-xs font-bold uppercase tracking-wider">Contratos</p>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
              ...
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">
                  {activeTab === 'staff' ? 'Vales por Descontar' : 'EPPs Entregados'}
              </p>
              <h3 className="text-3xl font-bold text-slate-800 mt-1">
                  {activeTab === 'staff' ? `S/ ${totalAdvances.toLocaleString()}` : Object.values(ppeCounts).reduce((a,b)=>a+b,0)}
              </h3>
              </div>
              <div className="w-12 h-12 bg-orange-50 rounded-full flex items-center justify-center text-orange-600">
                  {activeTab === 'staff' ? <Banknote size={24} /> : <HardHat size={24} />}
              </div>
          </div>
        )}
