...
const DashboardPage = () => {
  ...
  // Estados para tablas y modales auxiliares
  ...
  // 2. HELPER: Obtener fecha local (YYYY-MM-DD)
  const getLocalDateStr = () => {
    const now = new Date();
    return now.toLocaleDateString('en-CA'); // Formato ISO local
  };
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const { data: attendanceData, error: attendanceError } = await query.maybeSingle();
        
        if (attendanceError) console.error("Error cargando asistencia:", attendanceError);
  ...
  const handleCheckIn = async () => {
    ...
    if (!currentUser || !currentUser.id) {
        setBtnLoading(false);
        setNotification({ isOpen: true, type: 'error', title: 'Error', message: 'Usuario no identificado.' });
        return;
    }
    ...
    try {
      ...
      setNotification({
        ...
        message: `Asistencia iniciada a las ${now.toLocaleTimeString()}`
      });
  ...
  // --- MARCAR SALIDA (NUEVA LÓGICA CON MODAL) ---
  
  // 1. Al hacer click en el botón, solo abrimos el modal
  const handleCheckOutClick = () => {
    if (!todayRecord) return;
    setShowConfirmModal(true);
  };

  // 2. Función que ejecuta la salida real (llamada por el modal)
  const processCheckOut = async () => {
    setShowConfirmModal(false); // Cerramos modal
    setBtnLoading(true); // Mostramos loading en el botón principal
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1 bg-[#0F172A] p-8 rounded-3xl text-white relative overflow-hidden shadow-xl">
          <div className="relative z-10">
            ...
            <p className="text-blue-200 text-sm md:text-base max-w-lg">Bienvenido al panel de control de L&K.</p>
        ...
        <div className="w-full md:w-1/3 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col justify-between relative overflow-hidden">
            <div className="relative z-10">
                ...
                <p className="text-xs text-slate-500 mt-1">{todayRecord ? 'Registro de hoy detectado' : 'No has marcado entrada hoy'}</p>
            </div>

            <div className="mt-6 flex flex-col gap-3 relative z-10">
                {!todayRecord ? (
                    <button onClick={handleCheckIn} disabled={btnLoading} className="w-full py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold shadow-lg shadow-green-900/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                    ...
                    <div className="space-y-3">
                        ...
                        {/* BOTÓN SALIDA - AHORA LLAMA A handleCheckOutClick */}
                        <button onClick={handleCheckOutClick} disabled={btnLoading} className="w-full py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-900/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <ExpiredDocumentsTable refreshTrigger={refreshTrigger} onUpdate={handleOpenUpdateDocs} />
        <ValidWorkersTable refreshTrigger={refreshTrigger} onAssign={handleOpenAssignProject} />
      </div>
      ...
      {/* MODAL DE CONFIRMACIÓN DE SALIDA (ESTILO PERSONALIZADO) */}
      <AnimatePresence>
        {showConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-slate-500 text-sm mb-8 leading-relaxed">
                Estás a punto de registrar el fin de tu jornada laboral. <br/>¿Deseas continuar?
              </p>
              ...
              <div className="flex gap-3">
                ...
                <button 
                  ...
                  className="flex-1 py-3.5 bg-orange-500 text-white font-bold text-sm rounded-xl hover:bg-orange-600 shadow-lg shadow-orange-500/30 transition-all active:scale-95"
                >
                  Confirmar
                </button>
              </div>
...
const ExpiredDocumentsTable = ({ refreshTrigger, onUpdate }) => {
  ...
  useEffect(() => { fetchExpiredDocs(); }, [page, refreshTrigger]);
  ...
  const fetchExpiredDocs = async () => {
    ...
    try {
      ...
      const { data: documents, error } = await supabase.from('hr_documents').select('*').lte('expiration_date', nextMonthStr).order('expiration_date', { ascending: true }).range(from, to);
      if (error) throw error;
      ...
      if (documents.length > 0) {
        ...
        setDocs(documents.map(doc => ({...doc, owner_name: personMap[doc.person_id]?.full_name || 'Desconocido', person_data: personMap[doc.person_id]})));
      } else { setDocs([]); }
  ...
  return (
    <div className="bg-white rounded-2xl border border-red-100 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="px-6 py-4 border-b border-red-100 bg-red-50/30 flex justify-between items-center"><h3 className="font-bold text-red-900 flex items-center gap-2"><AlertTriangle className="text-red-600" size={20}/> Documentación Crítica</h3><span className="text-[10px] font-bold text-red-500 bg-red-100 px-2 py-0.5 rounded-full uppercase">Acción Requerida</span></div>
      <div className="overflow-x-auto flex-1">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-50">{loading ? <tr><td colSpan="4" className="px-4 py-6 text-center text-slate-400">Cargando...</td></tr> : docs.length === 0 ? <tr><td colSpan="4" className="px-4 py-6 text-center text-slate-400">Todo en orden.</td></tr> : docs.map(doc => (<tr key={doc.id} className="hover:bg-red-50/40"><td className="px-4 py-3 font-medium text-slate-700">{doc.owner_name}</td><td className="px-4 py-3 text-slate-600 truncate max-w-[100px]">{doc.doc_type}</td><td className="px-4 py-3">{getStatusBadge(doc.expiration_date)}</td><td className="px-4 py-3 text-right"><button onClick={() => onUpdate(doc.person_data)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-200"><RefreshCw size={16}/></button></td></tr>))}</tbody>
        </table>
      </div>
...
const ValidWorkersTable = ({ refreshTrigger, onAssign }) => {
  ...
  useEffect(() => { fetchValidWorkers(); }, [page, refreshTrigger]);
  ...
  const fetchValidWorkers = async () => {
    ...
    try {
      const from = page * ITEMS_PER_PAGE;
      const to = from + ITEMS_PER_PAGE - 1;
      const { data: people, error } = await supabase.from('workers').select('id, full_name, category, project_assigned').eq('status', 'Activo').range(from, to).order('full_name');
      if (error) throw error;
      ...
      if (people.length > 0) {
        ...
        setWorkers(people.map(person => {
           const hasExpired = docs.filter(d => d.person_id === person.id).some(d => d.expiration_date && d.expiration_date < todayStr);
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center"><h3 className="font-bold text-slate-800 flex items-center gap-2"><CheckCircle className="text-green-600" size={20}/> Estado del Personal</h3><span className="text-[10px] font-bold text-slate-500 bg-slate-200 px-2 py-0.5 rounded-full uppercase">Obreros Activos</span></div>
      <div className="overflow-x-auto flex-1">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-50">{loading ? <tr><td colSpan="4" className="px-4 py-6 text-center text-slate-400">Cargando...</td></tr> : workers.length === 0 ? <tr><td colSpan="4" className="px-4 py-6 text-center text-slate-400">Sin personal.</td></tr> : workers.map(worker => (<tr key={worker.id} className="hover:bg-slate-50"><td className="px-4 py-3 font-bold text-slate-700">{worker.full_name}</td><td className="px-4 py-3 text-slate-600 truncate max-w-[100px]">{worker.project_assigned || '-'}</td><td className="px-4 py-3">{worker.docStatus === 'OK' ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-100 text-green-700 text-[10px] font-bold border border-green-200"><CheckCircle size={10}/> AL DÍA</span> : <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-bold border border-slate-200"><Clock size={10}/> REVISAR</span>}</td><td className="px-4 py-3 text-right"><button onClick={() => onAssign(worker)} className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 border border-indigo-200"><Briefcase size={16}/></button></td></tr>))}</tbody>
        </table>
      </div>
      <div className="px-4 py-2 border-t border-slate-100 bg-slate-50 flex justify-end gap-2"><button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="p-1 rounded hover:bg-white text-slate-500"><ChevronLeft size={18}/></button><button onClick={() => setPage(p => p + 1)} disabled={!hasMore} className="p-1 rounded hover:bg-white text-slate-500"><ChevronRight size={18}/></button></div>
    </div>
...
const AssignProjectModal = ({ isOpen, onClose, worker, onSuccess }) => {
    ...
    const handleSave = async () => {
        setLoading(true);
        try { const { error } = await supabase.from('workers').update({ project_assigned: selectedProject }).eq('id', worker.id); if (error) throw error; onSuccess(); onClose(); } catch (error) { console.error(error); alert("Error al asignar"); } finally { setLoading(false); }
    };
